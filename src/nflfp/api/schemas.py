...
class SimulationIn(Schema):
    ...
    correlation_mode: str = Field(
        ...
        description=(
            ...
            "game script, so the joint distribution is genuinely correlated — "
            "but at the shipped tail factors that approximation costs little. "
            "Phase 6D measured 80% lineup coverage at 0.7943 against a nominal "
            "0.800 on 2,878 held-out lineups, and 90% at 0.8919.\n\n"
...
class SimulationAssumptionsOut(Schema):
    ...
    player_independence: bool = Field(
        description=(
            "True: outcomes were drawn independently. Teammates share an "
            "offence and opposing players share game script, so the true joint "
            "distribution is correlated. Measured on 2,878 held-out lineups "
            "(Phase 6D), the cost of ignoring that is small: 80% interval "
            "coverage 0.7943 against a nominal 0.800, 90% coverage 0.8919. "
            "Enabling correlation moves both past nominal rather than onto it. "
            "The residual dependence is concentrated in a QB drawn alongside "
            "his own receivers.\n\n"
